...
def compare_prediction_poisson_once():
    temp_poisson_lower = 0
    temp_poisson_upper = 0
    matches_lower = 0
    matches_upper = 0
    for i in range(0, len(List)):
        Data = pd.read_csv("premier_league_all_seasons_cleaned_testfile.csv", skiprows=0, skipfooter=430 - i,
                               engine='python')
        if List.iloc[i, 8] in Data.Time:
            if (Data.HomeTeam == List.iloc[i, 1]).sum() + (Data.AwayTeam == List.iloc[i, 2]).sum() <= LowerBound:
                matches_lower += 1
                if prediction_poisson(Data, List.iloc[i, 1], List.iloc[i, 2], 0.004, List.iloc[i, 8]) == List.iloc[i, 5]:
                    temp_poisson_lower += 1
        else:
            matches_upper += 1
            if prediction_poisson(Data, List.iloc[i, 1], List.iloc[i, 2], 0.004, "Unknown") == List.iloc[i, 5]:
                temp_poisson_upper += 1
    print(
        f"The Poisson Distribution got {round(Decimal(((temp_poisson_lower + temp_poisson_upper) / len(List)) * 100), 10)} % correct in total")
    print(len(List))
    if matches_lower > 0:
        print(
            f"The Poisson Distribution got {round(Decimal((temp_poisson_lower / matches_lower) * 100), 10)} % correct in the lower bound")
        print(matches_lower)
    if matches_upper > 0:
        print(
            f"The Poisson Distribution got {round(Decimal((temp_poisson_upper / matches_upper) * 100), 10)} % correct in the upper bound")
        print(matches_upper)
